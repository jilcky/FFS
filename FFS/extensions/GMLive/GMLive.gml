#define live_preinit
// Generated at 2018-01-20 12:17:06 (9190ms)
...
if (live_enabled) {
	...
	switch(l_tk[0]){
		...
		case 9:
			switch(l_tk[2]){
				...
				case 7:
					...
					var l__g6=gml_std_array_wget(this[0],this[2]);
					switch(l__g6[0]){
						case 9:
							switch(l__g6[2]){
								case 8:case 9:break;
		...
		case 24:
			...
			if(this[2]<this[3]){
				var l__g9=gml_std_array_wget(this[0],this[2]);
				if(l__g9[0]==27){
					gml_std_array_push(l_nodes,gml_node_comment_line(l__g9[1],l__g9[2]));
					this[@2]+=1;
				}
			}
			while(l_proc&&this[2]<this[3]){
				...
				switch(l__g10[0]){
					...
					case 29:
						switch(l__g10[3]){
							case false:
								if(gml_builder_build_line(this))return true;
								gml_std_array_push(l_nodes,gml_node_comment_block_pre(l__g10[1],l__g10[2],this[12],false));
								break;
							case true:
								this[@2]+=1;
								gml_std_array_push(l_nodes,gml_node_comment_block(l__g10[1],l__g10[2]));
								break;
						}
						break;
					default:
	...
	if(l_unknown){
		...
		if(this[2]<this[3]){
			...
			if(l__g20[0]==19){
				...
				this[@2]+=1;
				if(gml_builder_build_expr(this,0))return true;
				this[@12]=gml_node_set_op(l__g20[1],l__g20[2],l_x,this[12]);
	...
	if(this[2]<this[3]){
		var l__g23=gml_std_array_wget(this[0],this[2]);
		if(l__g23[0]==27){
			this[@2]+=1;
			this[@12]=gml_node_comment_line_post(l__g23[1],this[12],l__g23[2]);
		}
	}
	return false;
} else return false;
...
if (live_enabled) {
	...
	if(g_live_request_id==undefined){
		...
		if(l_now>g_live_request_time){
			...
			if(g_live_request_guid==undefined)g_live_request_id=http_get(g_live_request_url+"/init?password="+g_live_request_password+"&config="+os_get_config()+"&version="+string(101)); else g_live_request_id=http_get(g_live_request_url+"/update?guid="+string(g_live_request_guid));
		}
	}
}
...
if (live_enabled) {
	...
	if(l_map[?"version"]==undefined||l_map[?"version"]<101)show_error("Outdated GMLive server detected! Please update the included files from the extension.",false);
...
if (live_enabled) {
	...
	while(++l_i<l_n){
		...
		script_execute(g_live_update_script,l_fmap[?"name"],l_fmap[?"ident"],l_fmap[?"code"]);
	}
}
